...
class AddStaffDialog(tk.Toplevel):
    """GUI window for adding a new staff member, with a light/dark toggle."""

    def __init__(self, parent, on_staff_added=None, dark_mode=False):
    ...
    def _load_header_icon(self, label_widget):
        """Loads assets/staff.png into the header badge."""
        if not PIL_AVAILABLE:
            label_widget.configure(text="\U0001F9D1\u200D\u2695\ufe0f",
                                    font=("Segoe UI", 16))
            return

        image_path = os.path.join(
            os.path.dirname(__file__), "assets", "staff.png"
        )
    ...
    def create_widgets(self):
        ...
        subtitle_lbl = tk.Label(
            header, text="Enter the staff details below",
        ...
        # Position, full width
        position_col = tk.Frame(form)
        position_col.pack(fill="x")
        self._track(position_col, "body")
        self.position_entry = self._make_field(
            position_col, 0, "\U0001F4BC", "Position", "e.g. Nurse, Surgeon"
        )
        ...
        self.add_button = tk.Button(
            footer, text="+  Add Staff", command=self.add_staff,
    ...
    def add_staff(self):
        ...
        position = self._get_value(self.position_entry)
        ...
        if not position:
            messagebox.showerror(
                "Invalid Input", "Please enter the staff position.",
                parent=self,
            )
